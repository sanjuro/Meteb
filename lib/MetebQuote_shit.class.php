...
/**
 * FLAG: QUESTION
 * Functions not declared public or private non static 
 * Variables past as pointers to objects
 */
	...
	//The market data is required to do a quote
		/**
		 * FLAG: QUESTION
		 * Commence Date set to a past date, inlfuences age next.
		 */
	...
	function calc_pp2($main_sex, $main_dob, $second_life, $spouse_sex, $spouse_dob, $gp, $spouse_rev, $pri, $annuity, &$upload_date=0, &$inception_date=0)
	{
	...
	//The number of rows depends on how fast the probability of survival reduces to zero.
		...
		/**
		 * FLAG: QUESTION
		 * This makes both sexes 3 == male on the server??
		 */
		if ($main_sex=="M")
			$main_sex=2;
		else
	...
	function calc_annuity2($main_sex, $main_dob, $second_life, $spouse_sex, $spouse_dob, $gp, $spouse_rev, $pri, $pp, &$upload_date=0, &$inception_date=0)
	{
	...
	//Purchase Price -> Annuity Amount : Set $annuity=0 and specify a value for $pp
		...
		/**
		 * FLAG: QUESTION
		 * Find Commence Date, down the rabit hole
		 * What is Age Next ?
		 */
		...
		/**
		 * FLAG: QUESTION
		 * Explain the Commission calculation as it is showing up as 100% shouldn't be 1.5 % etc?
		 */
	...
	public static function generate($quote, $commission, $pp = 0, $annuity = 0.00)
	{
		...
		/**
		 * FLAG: QUESTION
		 * Why initialzie the DB connetion in a function what happens if the DB settings change?
		 */
